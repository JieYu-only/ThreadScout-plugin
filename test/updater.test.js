...
import { localizeCommitSubject, needsShell, PluginUpdater, selectRemote } from '../lib/updater.js'
...
test('更新日志将历史英文提交说明转换为中文', () => {
  assert.equal(localizeCommitSubject('feat: expand Guoba task management and scan records'), '功能：扩展锅巴多任务管理与扫描记录')
  assert.equal(localizeCommitSubject('fix: cross-check Tieba login state'), '修复：使用多接口交叉验证贴吧登录状态')
})
