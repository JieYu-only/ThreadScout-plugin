...
const COOKIE_ALLOWLIST = new Set([
  'BDUSS', 'BDUSS_BFESS', 'STOKEN', 'BAIDUID', 'BAIDUID_BFESS', 'BIDUPSID', 'PSTM', 'TIEBAUID',
  'MAWEBCUID', 'MCITY', 'BA_HECTOR', 'BDORZ', 'ZFY', 'USER_JUMP', '__bid_n', 'ab_sr'
])

export function normalizeTiebaCookie(input) {
  const source = String(input ?? '').trim().replace(/^cookie\s*:\s*/i, '')
  const values = new Map()
  for (const part of source.split(';')) {
    const split = part.indexOf('=')
    if (split < 1) continue
    const name = part.slice(0, split).trim().replaceAll('\\_', '_')
    const value = part.slice(split + 1).trim()
    if (COOKIE_ALLOWLIST.has(name) && value && !/[\r\n]/.test(value)) values.set(name, value)
  }
  if (!values.has('BDUSS')) throw new Error('Cookie 中缺少 BDUSS，请粘贴登录后的完整 Cookie 请求头')
  const preferredOrder = [...COOKIE_ALLOWLIST]
  const names = preferredOrder.filter(name => values.has(name))
  return { cookie: names.map(name => `${name}=${values.get(name)}`).join('; '), names }
}
...
export class AuthStore {
  ...
  setCookie(accountId, cookie) {
    const normalized = normalizeTiebaCookie(cookie)
    const all = this.readAll()
    all[accountId] = { cookie: normalized.cookie, updatedAt: new Date().toISOString(), authFailures: 0 }
    this.writeAll(all)
    return { names: normalized.names, count: normalized.names.length }
  }
