...
const COMMIT_TRANSLATIONS = new Map([
  ['initialize ThreadScout plugin', '初始化 ThreadScout 插件'],
  ['add rendered command help card', '新增图片渲染指令帮助卡片'],
  ['resolve account profile and thread scanning', '修复账号资料显示与主题扫描'],
  ['add safe in-chat plugin updates', '新增安全的聊天内插件更新'],
  ['add QR login and account management', '新增扫码登录与账号管理'],
  ['complete QR login and Windows updater', '完善扫码登录与 Windows 更新器'],
  ['use Tieba QR cookie exchange flow', '修复贴吧扫码登录凭证交换流程'],
  ['align update and restart feedback', '统一更新与重启反馈'],
  ['normalize imported Tieba cookies', '规范化导入的贴吧 Cookie'],
  ['sync Guoba configuration at runtime', '修复锅巴配置运行时即时同步'],
  ['improve Tieba access and update feedback', '改进贴吧访问与更新反馈'],
  ['cross-check Tieba login state', '使用多接口交叉验证贴吧登录状态'],
  ['expand Guoba task management and scan records', '扩展锅巴多任务管理与扫描记录']
])

export function localizeCommitSubject(subject) {
  const match = String(subject ?? '').match(/^(feat|fix|docs|refactor|test|chore):\s*(.+)$/i)
  if (!match) return subject
  const prefix = { feat: '功能', fix: '修复', docs: '文档', refactor: '重构', test: '测试', chore: '维护' }[match[1].toLowerCase()]
  return `${prefix}：${COMMIT_TRANSLATIONS.get(match[2]) ?? match[2]}`
}
...
export class PluginUpdater {
  ...
  async changeLog(before, after) {
    ...
    return output.split(/\r?\n/).map(line => line.trim()).filter(Boolean).map(line => {
      const [meta, subject = ''] = line.split('||')
      return `${meta} ${localizeCommitSubject(subject)}`
    })
  }
