...
export class TiebaAdapter {
  ...
  async getThread(thread) {
    try {
      ...
      if (!body) throw Object.assign(new Error(`贴吧帖子页面未解析到首帖正文（HTML ${html.length} 字节）`), { code: 'PARSE_ERROR' })
  ...
  async getTbs({ recordValidation = true } = {}) {
    ...
    try { data = await this.json(await this.request('https://tieba.baidu.com/dc/common/tbs')) } catch (error) {
      if (recordValidation && error.code === 'AUTH_EXPIRED') { try { Object.assign(error, this.onAuthResult?.(false)) } catch {} }
      throw error
    }
    if (!data.is_login || !data.tbs) {
      let profile = null
      try { profile = await this.fetchAccountProfile() } catch (error) {
      ...
      if ((profileData.is_login === true || Number(profileData.is_login) === 1) && data.tbs) {
        this.lastProfile = profile
        if (recordValidation) { try { this.onAuthResult?.(true) } catch {} }
        return data.tbs
      }
      try {
      ...
      const error = Object.assign(new Error(recordValidation ? '贴吧 Cookie 无效或已过期' : '贴吧服务端未确认登录状态；Cookie 仍保留，可继续测试扫描或重新绑定'), { code: recordValidation ? 'AUTH_EXPIRED' : 'AUTH_UNCONFIRMED' })
      if (recordValidation) { try { Object.assign(error, this.onAuthResult?.(false)) } catch {} }
      throw error
    }
    if (recordValidation) { try { this.onAuthResult?.(true) } catch {} }
    return data.tbs
  ...
  async getAccountProfile(options) {
    await this.getTbs(options)
    let profile = this.lastProfile ?? null
    this.lastProfile = null
    try {
      profile ??= await this.fetchAccountProfile()
    } catch (error) {
      if (!['PARSE_ERROR', 'REQUEST_REJECTED', 'ACCESS_DENIED'].includes(error.code)) throw error
    }
