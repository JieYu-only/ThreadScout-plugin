...
export class ThreadScoutDatabase {
  ...
  upsertThread(thread, result) {
    ...
    this.db.prepare(`INSERT INTO threads(platform,thread_id,forum_name,title,author,created_at,discovered_at,last_seen_at,score,decision,match_detail)
      VALUES(?,?,?,?,?,?,?,?,?,?,?) ON CONFLICT(platform,thread_id) DO UPDATE SET title=excluded.title,last_seen_at=excluded.last_seen_at,score=excluded.score,decision=excluded.decision,match_detail=excluded.match_detail`)
      .run(thread.platform, String(thread.id), thread.forum, thread.title, thread.author ?? null, thread.createdAt ?? null, now, now, result.score, result.decision, JSON.stringify(result))
  }
