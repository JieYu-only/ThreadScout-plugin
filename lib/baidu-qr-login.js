...
export class BaiduQrLogin {
  ...
  async finish(token) {
    const timestamp = Date.now()
    const params = new URLSearchParams({ v: String(timestamp), bduss: token, u: 'https%3A%2F%2Ftieba.baidu.com%2Findex.html', loginVersion: 'v4', qrcode: '1', tpl: 'tb', apiver: 'v3', tt: String(timestamp), traceid: '', callback: 'bd__cbs__threadscout' })
    await this.request(`${PASSPORT}/v3/login/main/qrbdusslogin?${params}`, { redirect: 'manual', headers: { referer: 'https://tieba.baidu.com/', 'sec-fetch-site': 'same-site', 'sec-fetch-mode': 'navigate', 'sec-fetch-dest': 'document', 'upgrade-insecure-requests': '1' } })
    await this.request('https://tieba.baidu.com/index.html', { headers: { referer: 'https://passport.baidu.com/' } })
