...
export class BaiduQrLogin {
  ...
  async finish(token) {
    const timestamp = Date.now()
    const params = new URLSearchParams({ alg: 'v3', apiver: 'v3', bduss: token, loginVersion: 'v4', qrcode: '1', time: String(Math.floor(timestamp / 1000)), tpl: 'mn', tt: String(timestamp), u: 'https://www.baidu.com/', v: String(timestamp), callback: 'bd__cbs__threadscout' })
    await this.request(`${PASSPORT}/v3/login/main/qrbdusslogin?${params}`, { redirect: 'manual', headers: { referer: 'https://www.baidu.com/' } })
